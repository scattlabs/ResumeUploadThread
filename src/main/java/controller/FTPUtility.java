...
public class FTPUtility {
	 ...
	 */
	public void uploadFile(String uploadFile, String destDir) throws FTPException {
		try {
			boolean success = ftpClient.changeWorkingDirectory(destDir);
			if (!success) {
				/*
				 * throw new FTPException(
				 * "Could not change working directory to " + destDir +
				...
				for (String folder : folders) {
					...
					if (!folder.equals("")) {
						...
						if (!created) {
							System.out.println("dadal create folder");
						} else {
							path = folder;
							System.out.println("belasil create folder");
						}
					}
				}
				folders = null;
			...
			outputStream = ftpClient.storeFileStream(uploadFile);
